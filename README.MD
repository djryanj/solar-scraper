...
- It installs dependencies with `npm ci`, runs `npm run check`, and runs the test suite with coverage.
...
- Release container tags include the exact version, the `major.minor` line, the `major` line, and `latest`.
