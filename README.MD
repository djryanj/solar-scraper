...
* ``ECUHOST=<your-ecu-host-or-ip>`` (required)

* ``PORT=<tcp-port-for-express>``

* ``USE_MQTT=<true|false>``

* ``USE_HA_MQTT=<true|false>``
...
* ``HA_MQTT_TOPIC=<base-homeassistant-mqtt-topic>``
---  

``ECUHOST`` (Required) 
The hostname or IP address of your APSolar ECU. 
* Default: ``192.168.1.1``

``PORT``
The port that the container listens on (internally). Be sure to change the port Docker forwards if you change this.
* Default: ``3000``

``USE_MQTT``
Set to **true** if you want to use basic MQTT. See the [MQTT](#mqtt) section for more information. **Be sure to also set MQTT_HOST**!!
* Default: ``false``

``USE_HA_MQTT``
Set to **true** if you want to use Home Assistant to auto-discover solar information via MQTT. Note that USE_MQTT and USE_HA_MQTT are independent; you do not need to set USE_MQTT to true if USE_HA_MQTT is true. **Be sure to also set MQTT_HOST**!!
* Default: ``false``

``MQTT_HOST``
Set to the IP address or DNS name of your MQTT server. Both USE_MQTT and USE_HA_MQTT use the same host.
* Default: ``test.mosquitto.org``

**NOTE: The default host could potentially expose information to a public server you don't want exposed. Be sure to set this if you set USE_MQTT or USE_HA_MQTT to true!**
**NOTE: At this time, providing a self-signed certificate file for TLS communications is not supported. If your MQTT server uses PKI from a well-known certificate authority such as letsencrypt (be sure that the WHOLE chain is sent), it should work. At this time, self-signed cert behaviour is untested.**

``MQTT_USERNAME``
If you need to supply a username for your MQTT server, set it here.
* Default: ``null/unset`` (works for anonymous MQTT servers)

``MQTT_PASSWORD``
If you need to supply a password for your MQTT server, set it here. Note that it will be visible in the logs. You could provide this via Docker secrets.
* Default: ``null/unset`` (works for anonymous MQTT servers)

``MQTT_TOPIC``
Base topic for standard MQTT data. See [MQTT](#mqtt) section for more information.
* Default: ``home/solar``

``HA_MQTT_TOPIC``
Base topic for Home Assistant auto-discovery. This must match the base topic in your Home Assistant configuration.yaml. 
**Note: this can only be a single label (e.g. "homeassistant" or "hass") or auto-discovery will fail.**
* Default: ``homeassistant``

### Informational Environment Variables
The following environment variables provide information into the scraper but do not affect operation beyond the cosmetic (they are exported with metrics and are visible on the browseable home page). 

``SITENAME``
 A friendly site name. Mostly visible on the home page of the scraper but also exported with metrics.
 * Default: ``Your House!``
 
``BUILDID``
The BUILDID is intended to be passed in from an automation pipeline such as Azure DevOps and should be equivalent to the git hash (this was mostly used during development; a future release may hide this for production builds). The BUILDID can be read from the local .git directory, but that is not present in the Docker container.
* Default: ``missingBuildId`` (when nothing is passed to Docker or the .git directory is not present locally); otherwise the 7-character git hash of the 

``BUILDNUMBER``
The BUILDNUMBER is intended to be passed in from an automation pipeline such as Azure DevOps and should be equivalent to the automated build number, e.g. 20190910.1.
* Default: ``local``

``SOURCEBRANCHNAME``
The SOURCEBRANCHNAME is intended to be passed in from an automation pipeline such as Azure DevOps and should be equivalent to the git branch that the container was built from, e.g. master.
* Default: ``local``
...
## Prometheus
Prometheus expects an endpoint at ``/metrics``, which is where it is. In addition to the custom metrics provided by this scraper, there are node.js-specific metrics exported as well. You can browse the endpoint if you wish.
...
``solar_inverter_grid_voltage{inverterId=<inverterId>}``: Current grid voltage output by the inverter in volts (V).

## MQTT

Although only one MQTT host is allowed, you can output 2 kinds of MQTT data. One is formatted for use by the [MQTT auto-discovery function in Home Assistant](https://www.home-assistant.io/docs/mqtt/discovery/), enabled by setting the ``USE_HA_MQTT`` environment variable to ``true``, and the other is a more generically-formatted. 
...
**NOTE: Do not use more than a single label (e.g. ``homeassistant`` or ``hass``) for the Home Assistant auto-discovery base topic or auto-discovery will fail.**
...
``/panels/<inverterId>/gridVoltage``: Current grid voltage output by the inverter in volts (V).
...
|``/solar_panel_<inverterID>/power``| Current power output of the panel in watts (W). | ``sensor.solar_panel_<#>_power``|
...
|``/solar_panel_<inverterID>/voltage``| Current grid voltage output by the inverter in volts (V). | ``sensor.solar_panel_<#>_voltage``|
...
The scraper will return a JSON-formatted string by browsing to ``/json``, e.g. ``http://docker-host:3000/json``. This is useful for consumption into a wide variety of downstream things.
